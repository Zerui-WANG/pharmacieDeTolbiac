...
if(!empty($_POST)){
	...
	if(empty($_POST['pseudo']) || !preg_match('/^[a-zA-Z0-9_]+$/', $_POST['pseudo'])){
		...
		$uti = $requete ->fetch();
			...
			if($uti){
				$errors['pseudo'] = 'Ce pseudo est dÃ©jÃ  pris';
	...
	if(empty($_POST['mail']) || !filter_var($_POST['mail'], FILTER_VALIDATE_EMAIL)){
		$errors['mail']="Votre mail n'est pas valide";
			...
			$uti = $requete ->fetch();
				...
				if($uti){
					$errors['mail'] = 'Cet email est dÃ©jÃ  utilisÃ© pour un autre compte';
	...
	if (empty($errors)){
			...
			// Pour quel type de session la personne veut s'inscrire ? 
			...
			// inscription d'un utilisateur et un vendeur
			
			else{

				$requete_vendeur=$pdo->prepare("INSERT INTO prevendeur SET ID_pvend = ?, mdp_pvend=?, mail=?");
				//$mdp_hash_vend = password_hash($_POST['mdp'], PASSWORD_BCRYPT);
...
<h7>Lors de l'inscription, il se pourrait que vous ne puissiez pas valider le mail d'inscription. Si cas précédent alors : veuillez copier le lien suivant :
http://p1web2020.metalman.eu/~groupeS/cai-wang/Session2_0/confirm.php?id=$uti_id&token=$token  \n
avec une modification de $uti_id qui a pour valeur l'id de l'utilisateur récupérer dans la BDD et $token a pour valeur le confirmation_token de l'utilisateur dans la BDD);"</h7>
...
<form action="" method="POST">
		<table>
			...
			<tr>
				<td><label>Retaper votre mot de passe</label></td>
			...
			<tr>
				<td><input type="submit" name="CrÃ©er un compte"> <input type="reset" name="Annuler"></td>
			</tr>
		</table>
	</form>